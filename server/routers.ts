  ...
  organization: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Link user to the organization as PROVIDER
    ...
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (!userRow[0]?.organizationId) return null;
      const orgRows = await db.select().from(organizations).where(eq(organizations.id, userRow[0].organizationId)).limit(1);
    ...
    update: protectedProcedure
      .input(z.object({
        name: z.string().min(1).max(255).optional(),
        type: z.enum(['provider', 'municipality', 'other']).optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { organizations } = await import('../drizzle/schema');
        const userRow = await db.select().from(users).where(eq(users.id, ctx.user.id)).limit(1);
        if (!userRow[0]?.organizationId) throw new TRPCError({ code: 'NOT_FOUND', message: 'No organization found' });
        await db.update(organizations)
          .set({ ...input, updatedAt: new Date() })
          .where(eq(organizations.id, userRow[0].organizationId));
        return { success: true };
      }),
    ...
    uploadLogo: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const key = `org-logos/user-${ctx.user.id}-${Date.now()}.${ext}`;
