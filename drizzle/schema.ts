...
export const media = mysqlTable("media", {
	...
	issueReportType: mysqlEnum(['none','corrective','punchlist']).default('none').notNull(),
	issueStatus: mysqlEnum(['open','corrected']).default('open').notNull(),
	issueWorkflowAction: mysqlEnum(['none','assign','review','rejected','accepted']).default('none').notNull(),
...
export const users = mysqlTable("users", {
	...
	role: mysqlEnum(['user','admin','webmaster','client','project_mgr']).default('user').notNull(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
