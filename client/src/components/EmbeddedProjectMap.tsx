      ...
      marker.addListener("click", () => {
        ...
        if (isVideo) {
          if (media.thumbnailUrl) {
            ...
            mediaDisplay = `
              <div style="position: relative; width: 100%; height: 400px; border-radius: 4px; margin-bottom: 12px; background: #000; cursor: pointer;" onclick="window.__openVideo && window.__openVideo('${media.id}')">
            ...
            mediaDisplay = `
              <div style="width: 100%; height: 400px; background: #1f2937; border-radius: 4px; margin-bottom: 8px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; cursor: pointer;" onclick="window.__openVideo && window.__openVideo('${media.id}')">
          ...
          mediaDisplay = `<img src="${imageUrl}" alt="${media.filename}" style="width: 100%; height: 400px; object-fit: cover; border-radius: 4px; margin-bottom: 12px;" onerror="this.src='${media.url}'" />`;
        }
        
        const content = `
          <div style="max-width: 600px; padding: 12px; background: #1a1a1a; border-radius: 8px;">
            ${mediaDisplay}
          ...
          <>
            ...
            <div className="mt-3 p-3 bg-muted/50 rounded-lg border border-border">
              <p className="text-sm text-muted-foreground">
                <span className="font-medium text-foreground">Tip:</span> Click on any GPS marker to view media details. Click "Full Screen" to view map video GPS points with enhanced navigation.
              </p>
            </div>
          </>
        )}
